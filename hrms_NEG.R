...
main <- function(filename,rtwin,mzwin) {
  require(xcms)
  require(data.table)
  targets <- read.table("./Negative_LipidList.csv" , header=T, sep=',')
...
signals_deviations <- function() { # the csv files must be in the active directory
  ...
  write.csv(results[[1]],file="signals_NEG.csv", row.names=F)
  write.csv(results[[2]],file="deviations_NEG.csv", row.names=F)
  return(results)
}
